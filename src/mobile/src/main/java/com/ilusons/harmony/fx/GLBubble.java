...
class GLBubble extends GLShape {
	...
	public void update(long dt, float ratioY) {
		...
		vertexBuffer.put(0, GLEx.normalizeGl(0, fromX, toX));
		vertexBuffer.put(1, GLEx.normalizeGl(centerY * ratioY, fromY, toY));
		for (int i=1; i<=POINTS_PER_CIRCLE; i++) {
			vertexBuffer.put(COORDS_PER_VERTEX * i, GLEx.normalizeGl((float) Math.sin(-Math.PI + step * i), fromX, toX));
			vertexBuffer.put(COORDS_PER_VERTEX * i + 1, GLEx.normalizeGl((float) Math.cos(-Math.PI + step * i) * ratioY, fromY, toY));
		}
		this.fromY = fromY;
	}
