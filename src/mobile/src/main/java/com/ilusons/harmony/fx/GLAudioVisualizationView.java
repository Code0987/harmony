...
import com.ilusons.harmony.ref.ColorEx;
...
public class GLAudioVisualizationView extends GLSurfaceView implements AudioVisualization, InnerAudioVisualization {
    ...
    static class Configuration {
        ...
        public Configuration(Context context, AttributeSet attrs, boolean isInEditMode) {
            ...
            try {
                ...
                layersCount = MathEx.clamp(layersCount, Constants.MIN_LAYERS_COUNT, Constants.MAX_LAYERS_COUNT);
                wavesCount = array.getInt(R.styleable.GLAudioVisualizationView_av_wavesCount, Constants.DEFAULT_WAVES_COUNT);
                wavesCount = MathEx.clamp(wavesCount, Constants.MIN_WAVES_COUNT, Constants.MAX_WAVES_COUNT);
                ...
                waveHeight = MathEx.clamp(waveHeight, Constants.MIN_WAVE_HEIGHT, Constants.MAX_WAVE_HEIGHT);
                bubbleSize = array.getDimensionPixelSize(R.styleable.GLAudioVisualizationView_av_bubblesSize, Constants.DEFAULT_BUBBLE_SIZE);
                bubbleSize = MathEx.clamp(bubbleSize, Constants.MIN_BUBBLE_SIZE, Constants.MAX_BUBBLE_SIZE);
                ...
                footerHeight = MathEx.clamp(footerHeight, Constants.MIN_FOOTER_HEIGHT, Constants.MAX_FOOTER_HEIGHT);
                bubblesPerLayer = array.getInt(R.styleable.GLAudioVisualizationView_av_bubblesPerLayer, Constants.DEFAULT_BUBBLES_PER_LAYER);
                bubblesPerLayer = MathEx.clamp(bubblesPerLayer, Constants.DEFAULT_BUBBLES_PER_LAYER_MIN, Constants.DEFAULT_BUBBLES_PER_LAYER_MAX);
            ...
            for (int i = 0; i < colors.length; i++) {
                layerColors[i] = ColorEx.normalize(colors[i]);
            }
            backgroundColor = ColorEx.normalize(bgColor);
        ...
        private Configuration(@NonNull Builder builder) {
            ...
            MathEx.clamp(bubblesPerLayer, Constants.DEFAULT_BUBBLES_PER_LAYER_MIN, Constants.DEFAULT_BUBBLES_PER_LAYER_MAX);
            layersCount = MathEx.clamp(layersCount, Constants.MIN_LAYERS_COUNT, Constants.MAX_LAYERS_COUNT);
    ...
    public static class ColorsBuilder<T extends ColorsBuilder> {
        ...
        public T setBackgroundColor(@ColorInt int backgroundColor) {
            this.backgroundColor = ColorEx.normalize(backgroundColor);
            return getThis();
        }
