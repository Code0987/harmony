...
import com.ilusons.harmony.ref.GLEx;
...
class GLWave extends GLShape {
    ...
    /**
     * Smooth coefficient for {@link MathEx#smooth(float, float, float)} method.
     */
    ...
    private void initVertices() {
        ...
        // center
        vertices[0] = GLEx.normalizeGl(0f, fromX, toX);
        vertices[1] = GLEx.normalizeGl(-1f, fromY, toY);

        // left bottom footer
        vertices[3] = GLEx.normalizeGl(-1f, fromX, toX);
        vertices[4] = GLEx.normalizeGl(-1f, fromY, toY);

        // left top footer
        vertices[6] = vertices[3];
        vertices[7] = GLEx.normalizeGl(0f, fromY, toY);

        // right top footer
        vertices[vertices.length - 6] = GLEx.normalizeGl(1f, fromX, toX);
    ...
    public void update(float dAngle) {
        ...
        if (coefficient == 0 && latestCoefficient > 0) {
            coefficient = MathEx.smooth(0, latestCoefficient, SMOOTH_A);
        }

        float val = (float) Math.sin(angle) * coefficient;
        if (prevVal > 0 && val <= 0 || prevVal < 0 && val >= 0) {
            coefficient = MathEx.smooth(coefficient, latestCoefficient, SMOOTH_A);
            waveX = random.nextFloat() * 0.3f * (random.nextBoolean() ? 1 : -1);
        }
        prevVal = val;
        int i = 0;
        double step = 1.0 / POINTS_PER_WAVE;
        float posX = GLEx.normalizeGl(waveX, fromX, toX);
        float posY = GLEx.normalizeGl(val, fromY, toY);
        for (float time = 0; time < 1 - step / 2; time += step) {
            vertices[COORDS_PER_VERTEX * i + 1 + SKIP] = angle;
            vertexBuffer.put(COORDS_PER_VERTEX * i + SKIP, MathEx.quad(time, vertices[6], posX, vertices[vertices.length - 6]));
            vertexBuffer.put(COORDS_PER_VERTEX * i + 1 + SKIP, MathEx.quad(time, vertices[7], posY, vertices[vertices.length - 5]));
            i++;
        }
    }
