 ...
 */
public class VisualizerDbmHandler extends DbmHandler<byte[]> implements VisualizerEx.OnFftDataCaptureListener,
    ...
    private final VisualizerEx visualizerWrapper;
    private float[] dbs;
    ...
    VisualizerDbmHandler(@NonNull Context context, int audioSession) {
        visualizerWrapper = new VisualizerEx(context, audioSession, this);
    ...
    protected void onDataReceivedImpl(byte[] fft, int layersCount, float[] dBmArray, float[] ampArray) {
        ...
        for (int i = 0; i < dataSize; i++) {
            ...
            dbs[i] = MathEx.magnitudeToDb(sqMag);
            float k = 1;
            if (i == 0 || i == dataSize - 1) {
                k = 2;
            }
